...
from sklearn.utils import Bunch, check_array, check_scalar
...
def _path_residuals(
    ...
    y_pred = X_test_coefs + intercepts
    ...
    for i in range(n_alphas):
        # 取出第 i 個 alpha 對應的預測結果與真實值
        y_pred_i = y_pred[:, :, i]  # shape: (n_samples,)
        print(f"y_pred_i shape: {y_pred_i.shape}")
        y_test_i = y_test[:, :]     # shape: (n_samples,)
        print(f"y_test_i shape: {y_test_i.shape}")

        # 分成 10 分位數區間（0~9）
        quantile_bins = pd.qcut(y_pred_i, q=10, labels=False, duplicates='drop')

        # 找出 Q9 和 Q0 的 index
        q9_idx = quantile_bins == 9
        q0_idx = quantile_bins == 0

        # 計算 Q9 和 Q0 的 y_test 平均差（spread）
        if q9_idx.any() and q0_idx.any():  # 確保兩邊都有樣本
            q9_mean = y_test_i[q9_idx].mean()
            q0_mean = y_test_i[q0_idx].mean()
            spread = q9_mean - q0_mean
        else:
            spread = np.nan  # 避免資料太少導致錯誤

        spread_list.append(spread)

    spread_array = np.array(spread_list)  # shape: (n_alphas,)
    '''
    y_pred = X_test_coefs + intercepts

# ✅ 確保 y_pred 是 3 維： (n_outputs, n_samples_test, n_alphas)
if y_pred.ndim == 2:
    y_pred = y_pred[np.newaxis, :, :]

# ✅ 確保 y_test 是 (n_samples, 1)
if y_test.ndim == 1:
    y_test = y_test[:, np.newaxis]
...
for i in range(n_alphas):
    y_pred_i = y_pred[0, :, i]  # 單輸出情況
    y_test_i = y_test[:, 0]
    n_samples = len(y_pred_i)

    print(f"\n[Alpha {i}] y_pred_i mean={np.mean(y_pred_i):.4f}, std={np.std(y_pred_i):.4f}")

    # 根據樣本數動態決定 quantile 數（每組至少 5 筆）
    min_per_bin = 5
    max_bins = n_samples // min_per_bin
    n_bins = min(10, max_bins)
    if n_bins < 2:
        print("  [SKIP] Too few samples to make quantile groups.")
        spread_list.append(np.nan)
        continue

    # 強制 rank 分箱（即使預測值一樣也能分）
    sorted_idx = np.argsort(y_pred_i)
    quantile_bins = np.empty_like(sorted_idx)
    bin_size = n_samples // n_bins
    for q in range(n_bins):
        start = q * bin_size
        end = (q + 1) * bin_size if q < n_bins - 1 else n_samples
        quantile_bins[sorted_idx[start:end]] = q

    q0_idx = quantile_bins == 0
    qN_idx = quantile_bins == (n_bins - 1)

    print(f"  Q0 size: {q0_idx.sum()}, Q{n_bins - 1} size: {qN_idx.sum()}")

    if q0_idx.sum() == 0 or qN_idx.sum() == 0:
        print("  [SKIP] Q0 or QN has 0 samples")
        spread_list.append(np.nan)
        continue

    q0_mean = y_test_i[q0_idx].mean()
    qN_mean = y_test_i[qN_idx].mean()
    spread = qN_mean - q0_mean
    spread_list.append(spread)
    print(f"  Spread (Q{n_bins - 1} - Q0): {spread:.4f}")

spread_array = np.array(spread_list)

if np.all(np.isnan(spread_array)):
    print("[ERROR] All spreads are NaN across all alphas!")
    raise ValueError("All spread values are NaN. Check if predictions have sufficient variation.")

print("\n[Result] Spread summary:")
print("Spread array:", spread_array)
print("Best spread:", np.nanmax(spread_array))
